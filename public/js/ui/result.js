...
import { createVideoPlayer, createMusicPlayer } from "../components/player.js";
import {
  truncate,
  showToast,
  stopAllMedia,
...
export function renderMediaSlides(container, items, resultThumbnail) {
  ...
  container.querySelectorAll(".astrostar-player-container, .astrostar-music-card").forEach((pc) => {
  ...
  items.forEach((dl, index) => {
    ...
    if (isVideo) {
      ...
      const musicPlayer = createMusicPlayer(dl, index, resultThumbnail);
      slide.appendChild(musicPlayer);
    } else {
      if (isDataSaver && !isLocal) {
        const placeholder = document.createElement("div");
        placeholder.className = "data-saver-placeholder";
...
export function renderResult(result, originalUrl) {
  ...
  if (sliderItems.length > 0) {
    const itemsToRender = isSinglePreview ? [sliderItems[0]] : sliderItems;
    renderMediaSlides(slidesWrapper, itemsToRender, result.thumbnail);
